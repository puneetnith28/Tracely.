...
- **Frontend App:** [https://Tracely.vercel.app/](https://Tracely.vercel.app/)
...
Tracely provides a simple **blockchain-style provenance system** where each product batch is assigned a **unique QR identity**.  
...
```js
localStorage.removeItem("Tracely-batches");
```
...
Data is persisted in browser **localStorage** (`Tracely-batches`).
...
- **Google Gemini**: Primary LLM for summarization, RAG lookups, and QA; backend entrypoint in [Tracely_backend/routes/ai.py](Tracely_backend/routes/ai.py) and configuration in [Tracely_backend/config.py](Tracely_backend/config.py).
<!-- - **Open Router**: LLM routing/proxy that lets us switch providers (Gemini, etc.) without code changes; used by the backend AI layer ([Tracely_backend/config.py](Tracely_backend/config.py), [Tracely_backend/routes/ai.py](Tracely_backend/routes/ai.py)). -->
- **Eleven Labs**: Text-to-speech for playback of verification timelines and notifications; implemented in [app/Tracely_mobile/services/tts.ts](app/Tracely_mobile/services/tts.ts).
- **Kiro**: Attestation/identity integration used for signer/actor verification and optional credential checks; integration points are client API flows and backend routes (see [app/Tracely_mobile/services/api.ts](app/Tracely_mobile/services/api.ts) and backend AI routes).
- **Insforge**: Image/asset-processing and augmentation provider used by the vision pipeline for image analysis/OCR to save in db and use Auth of it; referenced by [Tracely_backend/vision.py](Tracely_backend/vision.py).
- **Solana**: Target blockchain for on-chain proof anchoring; demo currently uses simulated ledger refs, with on-chain integration code in [Tracely_frontend/src/lib/web3.ts](Tracely_frontend/src/lib/web3.ts).
